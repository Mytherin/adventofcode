...
fn main() {
    ...
    // insert all beacons from scanner 0
    for point in &scanners[0].positions {
        all_beacons.insert(*point);
    }
    ...
    for xscanner in 1..scanners.len() {
        ...
        candidate_scanners.push(Scanner { index: xscanner, positions: scanners[xscanner].positions.clone() });
        let mut scanner_index = 0;
        let mut found = false;
        loop {
