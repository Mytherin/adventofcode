...
fn main() {
    ...
    let mut fishies : Vec<BigUint> = Vec::new();
    for _i in 0..MAX_FISH_COUNT {
        fishies.push(0.to_biguint().unwrap());
    }
    ...
    for line in lines {
        ...
        for split in splits {
            ...
            fishies[fish_count] += 1.to_biguint().unwrap();
        }
    }
    for _day in 0..DAY_COUNT {
        let mut new_fishies : Vec<BigUint> = Vec::new();
        for _i in 0..MAX_FISH_COUNT {
            new_fishies.push(0.to_biguint().unwrap());
        }
    ...
    let mut total_fish = 0.to_biguint().unwrap();
    for i in 0..MAX_FISH_COUNT {
