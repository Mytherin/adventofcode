...
fn add_node(name : String, nodes : &mut Vec<Node>) -> usize {
    ...
    let is_start_or_end = name == "start" || name == "end";
    nodes.push(Node { name : name, neighbors : Vec::new(), is_small : is_small, is_start_or_end : is_start_or_end});
    return nodes.len() - 1;
}

fn traverse_cave(current_pos : usize, goal : usize, mut visited : Vec<bool>, nodes : &Vec<Node>, double_visited_small_cave : bool) -> usize {
    if current_pos == goal {
        return 1;
    }
    ...
    for neighbor in &nodes[current_pos].neighbors {
        let mut double_visit = double_visited_small_cave;
        if visited[*neighbor] {
            if !double_visited_small_cave && !nodes[*neighbor].is_start_or_end {
                double_visit = true;
            } else {
        ...
        paths += traverse_cave(*neighbor, goal, visited.clone(), nodes, double_visit);
    }
    return paths;
}
...
fn main() {
    ...
    let path_count = traverse_cave(start_index, end_index, visited, &nodes, false);
    println!("{}", path_count);
}
