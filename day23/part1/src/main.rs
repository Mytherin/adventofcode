...
fn is_correct_room(s : Square, x : usize, y : usize) -> bool {
    if is_amber(s) && x == ROOM_START_X {
        return true;
    }
    if is_bronze(s) && x == ROOM_START_X + ROOM_OFFSET_X {
        return true;
    }
    if is_copper(s) && x == ROOM_START_X + ROOM_OFFSET_X * 2 {
        return true;
    }
    if is_desert(s) && x == ROOM_START_X + ROOM_OFFSET_X * 3 {
        return true;
    }
    return false;
}
...
impl State {
    ...
    fn is_finished(self : &State) -> bool {
        ...
        for _i in 0..ROOM_COUNT {
            if !is_dinosaur(self.get_state(x, y)) {
                return false;
            }
            if !is_correct_room(self.get_state(x, y), x, y) {
                return false;
            }
            if !is_matching_dinosaur(self.get_state(x, y), self.get_state(x, y + 1)) {
                return false;
            }
            x += ROOM_OFFSET_X;
        }
    ...
    fn possible_solution(self : &State, moving_dinosaur : Square, x : usize, y : usize) -> bool {
        ...
        // if this is the bottom we can always move there
        if y == ROOM_START_Y + 1 {
            return true;
        }
        // if this is not the bottom, the other dinosaur needs to match
        if is_matching_dinosaur(moving_dinosaur, self.get_state(x, y + 1)) {
            return true;
        }
        return false;
    }
    ...
    fn get_minimal_cost(self : &State) -> usize {
        // any dinosaur in the hallway will need to move to a room
        // this costs AT LEAST two steps
        let mut minimum_cost : usize = self.cost;
        for x in 0..WIDTH {
            let hall_state = self.get_state(x, HALLWAY_Y);
            if is_dinosaur(hall_state) {
                minimum_cost += dinosaur_move_cost(hall_state) * 2;
            }
        }
        // any room that contains two dinosaurs of different types
        // will need one of those dinosaurs to move
        // this costs AT LEAST four steps of the top dinosaur
        for x in 0..ROOM_COUNT {
            for y in 0..ROOM_HEIGHT {
                let dino_x : usize = ROOM_START_X + x * 2;
                let dino_y : usize = ROOM_START_Y + y;
                let top_state = self.get_state(dino_x, dino_y);
                let bottom_state = self.get_state(dino_x, dino_y + 1);
                if !is_matching_dinosaur(top_state, bottom_state) {
                    if is_dinosaur(top_state) && is_dinosaur(bottom_state) {
                        minimum_cost += dinosaur_move_cost(top_state) * 4;
                    }
                }
            }
        }
